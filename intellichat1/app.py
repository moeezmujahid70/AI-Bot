...
if user_input and user_input.strip():
    ...
    if len(messages) == 1 and messages[0]["is_user"]:
        ...
        cursor = conn.cursor()
        cursor.execute("UPDATE chats SET title = ? WHERE id = ?", (tag, st.session_state.current_chat_id))
        conn.commit()
        conn.close()

    prompt = user_input
    if st.session_state.use_context:
        rag_context = retrieve_similar_context(user_input, st.session_state.user_id)
