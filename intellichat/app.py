...
# Load custom CSS
def load_css():
    css_file = os.path.join(os.path.dirname(__file__), "templates", "style.css")
...
def display_chat_interface():
    ...
    with col1:
        ...
        for chat in chats:
            chat_class = "chat-item active-chat" if st.session_state.current_chat_id == chat["id"] else "chat-item"
    ...
    with col2:
        ...
        for msg in messages:
            if msg["role"] == "user":
                st.markdown(f"**You** ({format_time(msg['timestamp'])}): {msg['content']}")
            else:
                st.markdown(f"**Gemini** ({format_time(msg['timestamp'])}): {msg['content']}")
