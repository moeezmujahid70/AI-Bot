...
def process_message(user_id, chat_id, user_message, use_context=False):
    ...
    chat = genai.GenerativeModel('gemini-1.5-flash').start_chat(history=history[:-1])  # Exclude the latest message
    ...
    ai_message_id = save_message(chat_id, user_id, response_text, is_user=False)
